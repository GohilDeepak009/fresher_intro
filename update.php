...
<div class="header">
<p><a href="index.php">View Fresher</a> OR <a href="add.php">Introduce Myself</a> OR <a href="delete.php">Delete Intro</a></p>
</div>
<footer>
    <p class="attribution">
...
$query = "UPDATE `intro` SET `name` = '$name', `bio` = '$bio' WHERE `intro`.`id` = $id; ";

$result = mysqli_query($con,$query) or exit("Failed to run query");

if($result)
{
    echo '<script>alert("Your Details are Update Successfully");</script>';
}
else
{
    echo '<script>alert("Something went wrong while updating your details");</script>';
}
